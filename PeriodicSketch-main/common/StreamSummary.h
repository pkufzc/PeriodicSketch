...
public:
    ...
    static uint32_t Memory2Size(uint32_t memory) {
        // 假设每个 entry 占用：sizeof(pair<DATA_TYPE, DataNode*>) + 链表节点开销
        uint32_t entry_size = sizeof(std::pair<DATA_TYPE, DataNode*>) + 
                            sizeof(DataNode) + 
                            sizeof(CountNode) - sizeof(COUNT_TYPE); // 避免重复
        // 更简单：只按 cuckoo map 容量算
        uint32_t cuckoo_entry_size = sizeof(DATA_TYPE) + sizeof(DataNode*);
        return memory / cuckoo_entry_size; // 保守估计
    }
